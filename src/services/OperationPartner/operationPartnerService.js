...
export default {
    ...
    return new Promise((resolve, reject) => {
      request = request
        ...
        : {
            ...
            status: undefined,
            idOperationPartner: undefined
          };
      HTTP.get(
        config.apiEndpoints.operationPartnerUri.concat(
          `ListCustomers?page=${request.page}&pageItems=${
            request.pageItems
          }&searchWord=${request.searchWord}&sort=${request.sort}&idOperation=${
  ...
  changeCustomerStatus: request => {
    return new Promise((resolve, reject) => {
      HTTP.put(
        config.apiEndpoints.operationPartnerUri.concat(
          `UpdateCustomerStatus?idCustomer=${request.id}&status=${request.status}`
        )
      ).then(
        response => {
          resolve(response.data);
