    ...
    <div class="col-12">
      <card card-body-classes="table-full-width">
        <template slot="header">
          <h4 class="card-title">Pré-cadastro
          <base-link to="/customers/new" class="btn btn-icon btn-simple btn-twitter btn-sm"><i class="tim-icons icon-simple-add"></i></base-link>  
          </h4>
        </template>
        <div>
        ...
        <div slot="footer" class="col-12 d-flex justify-content-center justify-content-sm-between flex-wrap">
          <span class="btn btn-info mt-3 btn-simple btn-file" :loading="formLoading">
          ...
          <a href="/Exemplo-lista-clientes.xlsx" class="btn mt-3 btn-info btn-simple">Exemplo arquivo excel</a>
          <base-pagination class="pagination-no-border mt-3" v-model="pagination.currentPage" :per-page="pagination.perPage"
...
export default {
  ...
  methods: {
    ...
    validateModal() {
      ...
      this.$validator.validateAll('modalScope').then(isValid => {
        if (isValid) {
          ...
          operationService.deleteCustomer(self.$store.getters.currentUser.idOperation, self.modal.model.id).then(
                response => {
                self.$notify({
                    type: 'primary',
    ...
    uploadFile(event){
      ...
      operationService.uploadCustomerList(self.$store.getters.currentUser.idOperation, file).then(
            response => {
                self.$notify({
                    type: 'primary',
                    message: response.message,
