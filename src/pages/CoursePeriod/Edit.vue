...
<div class="row">
  <div class="col-md-12">
    <card title="Período">
      <h4 slot="header" class="card-title">Período</h4>
      ...
      <form class="form-horizontal" v-loading="formLoading" @submit.prevent>
        ...
        <div class="row">
          <div class="col-md-12">
            <base-link class="btn mt-3 btn-simple btn-primary" to="/coursePeriod">Voltar</base-link>
            <base-button 
              class="mt-3 pull-right" 
              native-type="submit" 
              type="info"
              @click.native.prevent="validatePeriod"
...
export default {
  ...
  computed: {
    viewAction() {
      return this.$route.name == 'edit_period' ? 'edit' : 'new';
    }
  },
  methods: {
    getError(fieldName) {
    ...
    savePeriod(vm) {
      ...
      if (vm.model.id === 0) {
        coursePeriodService.create(vm.model).then(
          res => {
            vm.$notify({
              type: 'success',
        ...
        coursePeriodService.update(vm.model).then(
          response => {
            vm.$notify({
              type: 'primary',
    ...
    fetchData() {
      ...
      if (self.viewAction == 'edit') {
        ...
        coursePeriodService.get(self.id).then(
          response => {
