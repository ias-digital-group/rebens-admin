<template>
  <div class="row">
    <div class="col-12">
      <card card-body-classes="table-full-width">
        <template slot="header">
          <h4 class="card-title">Tipo de Graduação
          <base-link to="/courseGraduationType/new" class="btn btn-icon btn-simple btn-twitter btn-sm"><i class="tim-icons icon-simple-add"></i></base-link>  
          </h4>
        </template>
        <div>
          <div class="col-12 d-flex justify-content-center justify-content-sm-between flex-wrap">
            ...
            <base-input>
              <el-input
                ...
                placeholder="Procurar Tipo de Graduação"
    ...
    <modal
      ...
      <h4 slot="header" class="title title-up">Remover Tipo de Graduação</h4>
...
export default {
  ...
  data() {
    return {
      internalName: 'tipo de graduação',
      sortField: 'name',
      tableColumns: [
        {
          prop: 'id',
  ...
  methods: {
    handleEdit(index, row) {
      this.$router.push(`/courseGraduationType/${row.id}/edit/`);
    },
    fetchData() {
      const self = this;
      const request = {
      ...
      courseGraduationTypeService.findAll(request).then(
        response => {
    ...
    validateModal() {
      ...
      this.$validator.validateAll().then(isValid => {
        if (isValid) {
          ...
          courseGraduationTypeService.delete(self.modal.model.id).then(
            response => {
              self.$notify({
                type: 'primary',
