<template>
  <div class="row">
    ...
    <modal :show.sync="modal.visible" headerClasses="justify-content-center">
      <h4 slot="header" class="title title-up">Remover Curso</h4>
      <form
        class="modal-form"
        ref="modalForm"
        @submit.prevent
...
export default {
  ...
  methods: {
    ...
    duplicate(index, row) {
      ...
      courseService.duplicate(row.id).then(
        response => {
          self.$data.loading = false;
          if (response.status === 'ok') {
            this.$router.push(`/course/${response.message}/edit/`);
          } else {
            self.$notify({
              type: 'primary',
    ...
    toggleStatus(index, row) {
      ...
      debugger;
      self.$data.loading = true;
      courseService.changeActive(row.id, !row.active).then(
        response => {
          if (response.status === 'ok') {
    ...
    fetchData() {
      ...
      courseService.findAll(request).then(
        response => {
    ...
    validateModal() {
      ...
      this.$validator.validateAll().then(isValid => {
        if (isValid) {
          ...
          courseService.delete(self.modal.model.id).then(
            response => {
              self.$notify({
                type: 'primary',
                message: response.message,
