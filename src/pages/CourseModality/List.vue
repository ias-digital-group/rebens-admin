<template>
  <div class="row">
    <div class="col-12">
      <card card-body-classes="table-full-width">
        <template slot="header">
          <h4 class="card-title">Modalidade
          <base-link to="/courseModality/new" class="btn btn-icon btn-simple btn-twitter btn-sm"><i class="tim-icons icon-simple-add"></i></base-link>  
          </h4>
        </template>
        <div>
    ...
    <modal
      ...
      <h4 slot="header" class="title title-up">Remover Modalidade</h4>
...
export default {
  ...
  methods: {
    handleEdit(index, row) {
      this.$router.push(`/courseModality/${row.id}/edit/`);
    },
    fetchData() {
      const self = this;
      const request = {
      ...
      courseModalityService.findAll(request).then(
        response => {
    ...
    validateModal() {
      ...
      this.$validator.validateAll().then(isValid => {
        if (isValid) {
          ...
          courseModalityService.delete(self.modal.model.id).then(
            response => {
              self.$notify({
                type: 'primary',
