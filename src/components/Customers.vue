<template>
  <div class="row">
    <div class="col-12" v-if="showTable">
      <el-table ref="customersTable" :data="tableData" v-loading="loading" empty-text="...">
    ...
    <div class="col-12 d-flex justify-content-center justify-content-sm-between flex-wrap" v-if="!showForm">
      ...
      <a class="btn mt-3 btn-success btn-simple" @click="showForm = true">Adicionar</a>
    </div>
    ...
    validateModal() {
      ...
      this.$validator.validateAll('modalScope').then(isValid => {
        if (isValid) {
          ...
          operationService.deleteCustomer(self.parentId, self.modal.model.id).then(
                response => {
                self.$notify({
                    type: 'primary',
